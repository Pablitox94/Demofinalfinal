...
const CargaDatosSuperior = () => {
  ...
  const handleVariableTypeSelection = (transcript) => {
    ...
    if (numericKeywords.some((keyword) => transcript.includes(keyword))) {
      ...
      toast.success('Modo numerico activado');
      return true;
    }
    ...
    toast.info('Deci "numeros" o "palabras" para continuar');
    return false;
  };
  ...
  const handleVoiceTranscript = (transcript) => {
    ...
    if (isQuantitativeType(variableType)) {
      ...
      if (numbers.length > 0) {
        ...
        toast.success(`Numeros detectados: ${numbers.join(', ')}`);
      } else {
        toast.error('No se reconocieron numeros validos');
      }
      return;
    }
    ...
    if (words.length > 0) {
      ...
      toast.error('No se reconocieron palabras validas');
    }
  };
